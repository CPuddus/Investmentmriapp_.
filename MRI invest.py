...
line_chart = alt.Chart(df).transform_fold(
    ...
    x=alt.X(
        "Year:O",
        title="Year",
        axis=alt.Axis(labelAngle=0, labelAlign="left")
    ),
    y=alt.Y(
        "Value:Q",
...
def create_pdf_enterprise():
    ...
    summary_text = [
        f"The total profit in {years} years is {format_currency(round(display_profit,0), currency_symbol, selected_currency)}.",
